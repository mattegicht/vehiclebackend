...
**Auth:** Stateless JWT. `JwtFilter` runs on every request, reads `Authorization: Bearer <token>`, calls `JwtUtil.parseClaims()`, then loads `UserDetails` and sets `SecurityContextHolder`. Tokens expire after 24 h. `SecurityConfig` registers an explicit `HttpStatusEntryPoint(UNAUTHORIZED)` so a missing/expired/malformed token yields **401** and a role failure yields **403** — without it Spring defaults to `Http403ForbiddenEntryPoint` and the two are indistinguishable, which breaks the client's session-expiry handling. CORS is restricted to the origins in `CORS_ALLOWED_ORIGINS` (comma-separated, default `https://vehiclebackend.duckdns.org`); `Retry-After` is added to `exposedHeaders` so cross-origin web builds can read the login-lockout wait; native apps send no Origin header and are unaffected.
...
### Fixed on discovery (frontend review 2026-08-05)
15. **Expired JWT answered 403, not 401** — with no `formLogin`/`httpBasic` to register an entry point, Spring fell back to `Http403ForbiddenEntryPoint`, so an expired token was indistinguishable from a permission failure. The Flutter client keys session expiry on 401, so a user past the 24 h TTL was stuck on a permanently failing vehicle list until a manual logout. Now `exceptionHandling().authenticationEntryPoint(new HttpStatusEntryPoint(UNAUTHORIZED))`.
16. **`Retry-After` unreadable cross-origin** — the 429 lockout response sent the header but CORS never exposed it (not safelisted), so a web build on an absolute `server_url` read null and dropped the wait time from the user's message. Now `config.setExposedHeaders(List.of(HttpHeaders.RETRY_AFTER))`.

...
public class VehicleService {
    ...
    public void deleteVehicle(Long id) {
        ...
        boolean isCreator = vehicle.getUser() != null && vehicle.getUser().getId().equals(user.getId());
        if (!isCreator && !user.getRole().equals("ROLE_ADMIN")) {
    ...
    public Vehicle toggleInUse(Long id) {
        Vehicle vehicle = vehicleRepository.findById(id)
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "Vehicle not found"));
        User user = currentUser();
        if (vehicle.isInUse() && vehicle.getInUseBy() != null && !vehicle.getInUseBy().getId().equals(user.getId())) {
